  ...
  try {
    ...
    if (argc == ARGUMENTS_SIZE) {
      ...
      if (reportType == "cppcheck") {
        ...
        sonarqubeReport = Convertor::cppCheckToSonarReport(cppCheckReport);

      } else if (reportType == "clang-tidy") {
        sonarqubeReport = Convertor::clangTidyToSonarReport(inputFile);
      }
      ...
      std::cerr << "Bad usage of tool : " << std::endl
                << "  example of expected usage : cpp-coso "
