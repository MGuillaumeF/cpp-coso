...
const std::map<ESonarCloudSeverity, std::string, std::less<>>
    SonarCloudSeverityValue = {{ESonarCloudSeverity::INFO, "INFO"},
...
boost::property_tree::ptree buildLocationTree(const std::string &message,
...
Convertor::clangTidyToSonarReport(const std::string &filename) {
  ...
  for (auto i = std::sregex_iterator(reportContent.begin(), reportContent.end(),
    ...
    issue.put<std::string>(
        "severity",
        SonarCloudSeverityValue.at(clangTidyToSonarCloudSeverity.at(severity)));
